...
higher_risk_message = RiskMessage.create(:message => "Older women are at higher risk for breast cancer, especially after menopause. Discuss screening and risk reduction steps with your physician.")
lower_risk_message = RiskMessage.create(:message => "Older women are at higher risk for breast cancer, but women at any age can get the disease. Talk with your physician to learn steps you can take to reduce your risk AT ANY AGE.")

age_question = Question.create(:question_name => "How old are you?", :group_id => 1, :input_type => select, :category => health_history_category)
...
race_question = Question.create(:question_name => "With which race/ethnicity do you identify?", :group_id => 2, :input_type => radio, :category => health_history_category)
...
### Children
higher_risk_message = RiskMessage.create(:message => "Having no children or having a first child after 35 increases breast cancer risk, because your lifetime exposure to estrogen is higher with no or later pregnancies. Most risks for breast cancer are associated with lifetime exposure to estrogen.")
lower_risk_message = RiskMessage.create(:message => "Having children before age 35 decreases breast cancer risk. Keep reading and talk with your doctor about steps you can take to reduce your risk.")

children_question = Question.create(:question_name => "How old were you when your first child was born?", :group_id => 3, :input_type => radio, :category => health_history_category)
...
### Breastfeeding
higher_risk_message = RiskMessage.create(:message => "Breastfeeding for less than 12 months across all children increases your exposure to estrogen. Most risks for breast cancer are associated with lifetime exposure to estrogen.")
lower_risk_message = RiskMessage.create(:message => "Breastfeeding for more than 12 months across all children decreases your risk, and it protects your child's health, too.")

breastfeeding_question = Question.create(:question_name => "How many months (total) did you breast feed across all your children?", :group_id => 4, :input_type => radio, :category => health_history_category)
...
### Relatives
higher_risk_message = RiskMessage.create(:message => "Women have higher breast cancer risk if first degree relatives (mom, dad, sister, brother, son, daughter) have a history of breast cancer.")
# lower_risk_message = RiskMessage.create(:message => "Women have higher breast cancer risk if first degree relatives (mom, dad, sister, brother, son, daughter) have a history of breast cancer. ")
...
height_question = Question.create(:question_name => "How tall are you (in feet and inches)?", :group_id => 6, :input_type => select, :category => health_history_category)

height_option_choice = OptionChoice.create(:option_choice_name => "Less than 4'")
...
for feet in 4..6
  for inches in 0..11
    ...
    height_option_choice = OptionChoice.create(:option_choice_name => name)
    QuestionOption.create(:question => height_question, :option_choice => height_option_choice, :risk_level => no_risk_level)
  end  
end

height_option_choice = OptionChoice.create(:option_choice_name => "More than 6'")
...
weight_question = Question.create(:question_name => "How much do you weigh (in pounds)?", :group_id => 6, :input_type => select, :category => health_history_category)

weight_option_choice = OptionChoice.create(:option_choice_name => "Less than 90")
...
for weight in 90..299
  weight_option_choice = OptionChoice.create(:option_choice_name => weight)
  QuestionOption.create(:question => weight_question, :option_choice => weight_option_choice, :risk_level => no_risk_level)
end

weight_option_choice = OptionChoice.create(:option_choice_name => "300+")

QuestionOption.create(:question => weight_question, :option_choice => weight_option_choice, :risk_level => no_risk_level)


### Diet Habits
higher_risk_message = RiskMessage.create(:message => "Eating a diet with at least 5 servings of fruits and vegetables each day has been shown to be protective against developing breast cancer. Eat those fruits and veggies!")
lower_risk_message = RiskMessage.create(:message => "Wow! Your dietary habits are on target! Eating a diet with at least 5 servings of fruits and vegetables each day has been shown to be protective against developing breast cancer. Keep up the good work!")

diet_habits_question = Question.create(:question_name => "How would you describe your dietary habits?", :group_id => 7, :input_type => radio, :category => diet_category)
...
fresh_or_frozen_question = Question.create(:question_name => "Do you most often eat fresh/frozen foods or canned?", :group_id => 8, :input_type => radio, :category => diet_category)
...
### Charred Meat
higher_risk_message = RiskMessage.create(:message => "Eating char grilled food exposes you to dangerous chemicals called PAHs (polycyclic aromatic hydrocarbons) that have been linked to increased risk for breast cancer. Avoid eating charred meat.")
lower_risk_message = RiskMessage.create(:message => "Great! Eating char grilled food exposes you to chemicals that have been linked to increased risk for breast cancer, so keep avoiding them when possible.")

charred_meat_question = Question.create(:question_name => "Do you regularly eat charred meat?", :group_id => 9, :input_type => radio, :category => diet_category)
...
### Alcohol
higher_risk_message = RiskMessage.create(:message => "Having one or more servings of alcoholic drinks per day increases risk for breast cancer. For women who drink more than one serving per day, their risk is 1.5 times higher than for nondrinkers.")
lower_risk_message = RiskMessage.create(:message => "Just getting a drink out with friends every now and then? Not to worry; but having one or more servings of alcoholic drinks per day increases women's risk for breast cancer.")

alcohol_question = Question.create(:question_name => "Do you frequently have more than 8 alcoholic drinks per week?", :group_id => 10, :input_type => radio, :category => diet_category)
...
### Exercise
higher_risk_message = RiskMessage.create(:message => "Regardless of a woman's height and weight, regular exercise for 30 minutes or more, most days of the week, has been shown to reduce a woman's risk of breast cancer and results in numerous other health benefits. No more couch potato!")
lower_risk_message = RiskMessage.create(:message => "Wow! Good job on the fitness! By exercising more than 30 minutes most days of the week, you're reducing your risk for breast cancer and getting so many other health benefits.")

exercise_question = Question.create(:question_name => "How many days per week do you exercise 30 minutes or more?", :group_id => 11, :input_type => radio, :category => exercise_category)
...
### Fragrances
higher_risk_message = RiskMessage.create(:message => "Phthalates (pronounced tha-lates) are chemicals found in some personal care products like fragrances, nail polish and hair care products. When possible, use fragrance-free products.")
lower_risk_message = RiskMessage.create(:message => "Great! Keep using fragrance-free products when possible.")

fragrances_question = Question.create(:question_name => "Do you most often use personal care products like cosmetics with or without fragrances?", :group_id => 12, :input_type => radio, :category => environment_category)
...
lower_risk_message = RiskMessage.create(:message => "Great! Keep using glass containers for food whenever you can.")

materials_question = Question.create(:question_name => "Do you store, serve or microwave food in plastic or glass dishes?", :group_id => 13, :input_type => radio, :category => environment_category)
...
### Hormones
higher_risk_message = RiskMessage.create(:message => "Oral contraceptives and hormone replacement therapy (HRT) increase a woman's lifetime exposure to estrogen, increasing her risk for breast cancer. Avoid prolonged use of menopausal HRT, unless recommended by your doctor. ")
lower_risk_message = RiskMessage.create(:message => "Great! Oral contraceptives and hormone replacement therapy (HRT) increase a woman's lifetime exposure to estrogen, so avoid prolonged exposure when possible.")

hormones_question = Question.create(:question_name => "Have you used birth control for 10+ years or hormone replacement therapy for 5+ years?", :group_id => 14, :input_type => radio, :category => environment_category)
...
higher_risk_message = RiskMessage.create(:message => "Know the normal look and feel of your breasts so that it is easier to spot possible abnormalities.")
lower_risk_message = RiskMessage.create(:message => "Great! Knowing the normal look and feel of your breasts makes it easier to spot possible abnormalities.")

screening_question = Question.create(:question_name => "Do you know the normal look and feel of your breasts?", :group_id => 15, :input_type => radio, :category => screening_category)
...
higher_risk_message = RiskMessage.create(:message => "With your physician, discuss your family history, annual exams and the age to start mammograms.")
lower_risk_message = RiskMessage.create(:message => "Great! Be sure to discuss your family history, annual exams and the age to start mammograms.")

screening_question = Question.create(:question_name => "Have you talked with your personal care physician about your breast health?", :group_id => 16, :input_type => radio, :category => screening_category)

QuestionOption.create(:question => screening_question, :option_choice => yes_option_choice, :risk_level => lower_risk_level, :risk_message => lower_risk_message)
QuestionOption.create(:question => screening_question, :option_choice => no_option_choice, :risk_level => higher_risk_level, :risk_message => higher_risk_message)

### Screening: breast exams
higher_risk_message = RiskMessage.create(:message => "Annual breast exams by a physician and mammograms help detect breast cancer early, giving women a better chance for survival. US Preventive Services Task Force recommends that women ages 50-74 get mammograms every two years. ")
lower_risk_message = RiskMessage.create(:message => "Great! Annual breast exams by a physician and mammograms help detect breast cancer early, giving women a better chance for survival.")

screening_question = Question.create(:question_name => "Have you begun getting clinical breast exams and/or mammograms?", :group_id => 17, :input_type => radio, :category => screening_category)
...
higher_risk_message = RiskMessage.create(:message => "Having a Body Mass Index (BMI) of 25 or higher increases your risk for breast cancer. Keep reading to learn about changes you can make to your diet and exercise routines to maintain a healthy weight and lower your risk.")
lower_risk_message = RiskMessage.create(:message => "Congratulations! By maintaining your BMI between 18.5 and 24, you're reducing your risk for postmenopausal breast cancer. Keep reading for more ideas on steps in maintaining a healthy lifestyle and reducing breast cancer risk.")

bmi_question = Question.create(:question_name => "BMI", :group_id => 100, :input_type => select, :category => health_history_category)

bmi_option_choice = OptionChoice.create(:option_choice_name => "Underweight (< 18.5)")
...
bmi_option_choice = OptionChoice.create(:option_choice_name => "Normal (18.5 - 24.9)")
QuestionOption.create(:question => bmi_question, :option_choice => bmi_option_choice, :risk_level => lower_risk_level, :risk_message => lower_risk_message)

bmi_option_choice = OptionChoice.create(:option_choice_name => "Overweight (25 - 29.9)")
...
bmi_option_choice = OptionChoice.create(:option_choice_name => "Obese (30+)")
QuestionOption.create(:question => bmi_question, :option_choice => bmi_option_choice, :risk_level => higher_risk_level, :risk_message => higher_risk_message)